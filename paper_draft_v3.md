...
The continuing digitalization of power infrastructure has turned conventional grids into cyber-physical power systems (CPPS), in which the physical energy-delivery layer and the cyber sensing-and-control layer are tightly interdependent [Lit-CPPS-Overview-1, Lit-CPPS-Overview-2]. Wide-area measurement systems (WAMS), supervisory control and data acquisition (SCADA), and remedial action schemes (RAS) all rely on real-time exchange of measurements and dispatch commands between substations and one or more control centers (CCs). This integration improves observability and dispatch flexibility under normal operation, but it also opens a bidirectional failure channel: physical-layer disturbances can damage the communication topology that closes the control loop, while cyber-layer degradation can in turn destabilize generator dispatch and provoke further physical failures [Lit-Interdependence-1, Lit-Interdependence-2]. Resilience analysis in CPPS therefore must move beyond purely topological robustness and explicitly account for *control-loop timing quality* under stress [Lit-Resilience-Framework-1].

A large body of literature has modeled cascading failures in coupled power-communication systems through interdependency graphs, flow-based propagation rules, and vulnerability-oriented attack analyses [Lit-Cascade-Model-1, Lit-Cascade-Model-2, Lit-Attack-Strategy-1]. These studies established important foundations for understanding how failure traverses the inter-layer boundary. In most of these frameworks, however, communication delay is reported as an *outcome variable* — for instance, the path-length increment that follows topology damage — rather than enforced as a *causal variable* that actively reshapes the cascade trajectory itself [Lit-Delay-Indicator-1, Lit-Delay-Indicator-2]. Two limitations follow. First, because the cascade is allowed to unfold under ideal control and delay is appended only after the fact, every delay scenario inherits an identical structural failure sequence and delay merely rescales the final score; the question of *which* delay components drive the damage cannot be answered by construction. Second, the standard tolerance-margin parameter $\alpha$ that governs branch and node capacity is treated as a purely structural quantity, even though, in real engineering practice, a system designed with greater $N{-}k$ capacity reserves typically also provisions greater communication redundancy (IEC 61850-90-4 PRP, ITU-T G.8032 ring protection, NERC CIP-012 cross-CC backup channels), wider control dead-bands (IEEE C37.118 PMU reporting cadence, NERC PRC-024 frequency tolerance windows), and longer permissible waiting times before forced generator separation (NERC PRC-006-5, NASPI WAMS roadmap). Decoupling $\alpha$ from these cyber-physical co-benefits hides the very mechanism by which capacity margin buys the system additional tolerance against communication delay.

This paper closes both gaps within a single delay-aware cascading framework. We embed communication delay as an endogenous driver of every cascade round: before each DC power flow, every surviving generator's reference output is multiplied by a *control-efficiency factor* $\eta_i^{+}$ derived from the current state of the communication network and the generator's coupled cyber path to its nearest reachable control center. Rather than the legacy linear form $(1-k_m\tau_{m,i})(1-k_e\tau_{e,i})$, $\eta_i^{+}$ is decomposed into three physically grounded multiplicative channels: a saturation factor $\Phi_{\text{sat}}$ that captures dead-band-bounded measurement and execution lag (anchored to PMU reporting and AVR/governor dead bands), a loss factor $\Phi_{\text{loss}}$ that accounts for queueing-induced packet loss aggregated over hop count and parallel cyber-redundancy paths (IEEE Std 493 / IEC 61078 reliability block diagrams), and a critical factor $\Phi_{\text{crit}}$ that models the heterogeneous, capacity-dependent breakdown of generator controllability when total round-trip latency approaches the unit's transient-stability tolerance window (WAMS literature, 0.7–1.5 s). The same tolerance parameter $\alpha$ that scales branch and node capacity is then re-used as a triple lever onto $\eta_i^{+}$ — broadening the saturation dead band, lifting the parallel redundancy degree, and widening the critical tolerance window — together with an $\alpha$-coupled cap on under-frequency load shedding (UFLS) over-shedding that mirrors the increased frequency-response margin of higher-reserve systems. By construction, every history at $\alpha=0$ regresses bit-identically to the legacy formulation, so the cyber-physical coupling cannot be confounded with parameter retuning.

This delay-driven inner mechanism is wrapped in a unified evaluation protocol that addresses the second-order question of *how much* delay is in scope. The IEEE 39-bus power grid is coupled to a Barabási–Albert scale-free communication network through a betweenness-assortative one-to-one bus-to-cyber-node mapping, and control centers are placed by a Multi-attribute Structural Importance Score (MSIS) that fuses betweenness, $k$-shell index, closeness, degree, and clustering. A targeted attack removes the highest-betweenness non-CC node and initiates a nested dual-loop cascade: an inner structural-propagation loop resolves bidirectional cyber–power dependency damage to fixed point, and an outer overload loop applies $\eta_i^{+}$, runs DC power flow with optional UFLS, and trips overloaded branches. Five graded delay scenarios — *no_delay, light, baseline, medium, heavy* — are anchored to documented operating regimes (IEEE C37.118.1 PMU streams, IEC 61850-90-5 class M2/M3, NASPI cross-region P95 band, $\sim$60 % of NERC PRC-002-2 upper bound) and explicitly *not* tuned to chase metric values. Across 1,000 independent Monte Carlo realizations, eleven tolerance levels, and the five scenarios, two complementary robustness metrics are reported: $R_1$, a delay-adjusted retained-load ratio computed from a capacity-weighted in-service-only sample stream that respects the UFLS shedding cap, and $R_3$, a capacity-weighted normalized RMS deviation of actual generator dispatch from delay-free references, evaluated as a per-round cumulative trajectory rather than a terminal-only snapshot.

The principal contributions of this paper are as follows.

1. **Multi-channel delay-to-control mapping that closes the cascade feedback loop.** We replace the legacy linear control-efficiency factor with $\eta_i^{+}=\Phi_{\text{sat}}\cdot\Phi_{\text{loss}}\cdot\Phi_{\text{crit}}$, anchored to PMU/AVR dead bands, queueing-loss reliability theory, and unit-capacity-heterogeneous transient-stability windows, and apply it before every DC power flow so that delay actively drives the structural cascade rather than rescaling its outcome.
2. **Cyber-physical re-interpretation of the tolerance parameter $\alpha$.** We re-purpose $\alpha$ as a triple lever that simultaneously broadens the saturation dead band, lifts the cyber parallel-redundancy degree, and widens the critical tolerance window, and we $\alpha$-couple the UFLS over-shedding cap to the system's frequency-response reserve. The construction preserves bit-identical regression at $\alpha=0$.
3. **A unified, physically anchored graded-delay protocol with two complementary robustness metrics.** Five scenarios anchored to NERC, NASPI, IEC and IEEE references are evaluated under a consistent 1,000-trial Monte Carlo protocol, and robustness is jointly measured by the system-level delay-adjusted load retention $R_1$ and the generator-level capacity-weighted dispatch deviation $R_3$, the latter exposing hidden functional degradation invisible to load-based metrics alone.
4. **Per-round mechanism attribution and asymmetric vulnerability mapping.** $R_1$ and $R_3$ are reported not only as terminal scalars but also as per-round cumulative trajectories and tolerance–round heatmaps, revealing that the delay penalty concentrates asymmetrically at moderate-to-high tolerance margins and late cascade rounds — precisely where delay-free analysis predicts safety but delay-burdened systems collapse.

The remainder of the paper is organized as follows. Section 2 details the cyber-physical power system model: the IEEE 39-bus power grid, the BA cyber layer with MSIS-based CC placement, the role-aware bidirectional delay decomposition, the betweenness-assortative coupling, and the capacity-tolerance model. Section 3 presents the delay-integrated cascading dynamics, including the attack model, the inner structural-propagation loop, the outer overload-and-delay loop with $\eta_i^{+}$ injection and UFLS, and the closed-loop feedback mechanism. Section 4 defines the robustness metrics $R_1$ and $R_3$ together with the unified Monte Carlo protocol. Section 5 reports graded-scenario robustness results across $\alpha$. Section 6 carries out mechanism-attribution analysis decomposing degradation into $\Phi_{\text{sat}}$, $\Phi_{\text{loss}}$, $\Phi_{\text{crit}}$, path-length, and reachability factors. Section 7 discusses limitations and extensions, and Section 8 concludes.
